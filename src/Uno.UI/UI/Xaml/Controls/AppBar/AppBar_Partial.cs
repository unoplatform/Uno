...
#pragma warning restore CS0649
		...
		protected bool GetShouldOpenUp()
		{
			...
			if (m_Mode == AppBarMode.Inline)
			{
				...
				{
					layoutBounds = Xaml.Window.Current.Bounds;
		...
		private void SetFocusOnAppBar()
		{
			...
			if (!isAncestorOf)
			{
				if (m_savedFocusedElementWeakRef is { })
				{
					m_savedFocusedElementWeakRef.SetTarget(focusedElement);
				}
				else
				{
		...
		private void RestoreSavedFocus()
		{
			...
			if (m_savedFocusedElementWeakRef is { }
				&& m_savedFocusedElementWeakRef.TryGetTarget(out var savedFocusedElement))
			{
				RestoreSavedFocusImpl(savedFocusedElement, m_savedFocusState);
			}

			m_savedFocusedElementWeakRef?.SetTarget(null);
			m_savedFocusState = FocusState.Unfocused;
		}
		...
		private void TryQueryDisplayModesStatesGroup()
		{
			if (m_tpDisplayModesStateGroup == null)
			{
				GetTemplatePart<VisualStateGroup>("DisplayModeStates", out m_tpDisplayModesStateGroup);
		...
		private void UpdateOverlayElementBrush()
		{
			...
			if (m_isOverlayVisible)
			{
				...
				if (Resources.TryGetValue("AppBarLightDismissOverlayBackground", out var oBrush)
					&& oBrush is Brush brush)
				{
